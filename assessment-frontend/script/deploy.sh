...
ssh -oStrictHostKeyChecking=no -i ssh_key "ec2-user@${SERVER_HOST_NAME}" \
  ...
  && sudo docker pull 256909349812.dkr.ecr.eu-west-1.amazonaws.com/appops-ionica-frontend:ionicda001 \
  && sudo docker run --init -d --name live-container -p 80:3000 256909349812.dkr.ecr.eu-west-1.amazonaws.com/appops-ionica-frontend:ionicda001 \
  && sudo docker system prune -af" # remove unused images to free up space"
